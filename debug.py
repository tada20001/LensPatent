...
# --- 1. 페이지 설정 ---
st.set_page_config(page_title="디버그용 특허 검색 시스템", layout="wide")
st.title("🐞 DEBUG: 특허 검색 및 분석 시스템")
...
with main_tab_search:
    ...
    if 'search_result' in st.session_state and st.session_state.search_result:
        ...
        last_searched_params = st.session_state.search_result["search_params"]
        search_time = st.session_state.search_result["search_time"]

        # ★★★ 여기에 안전장치 추가 ★★★
        current_params = get_search_params()
        params_are_different = (last_searched_params != current_params)

        if params_are_different:
            st.warning("⚠️ 검색 조건이 변경되었습니다. 정확한 다운로드를 위해 '검색 결과 확인' 버튼을 다시 눌러주세요.")
        ...
        if total_hits > 0:
            ...
            # ★★★ 조건이 다르면 버튼을 비활성화 시킴 ★★★
            if st.button(f"엑셀 다운로드 및 대시보드 생성 ({download_count:,} 건)", use_container_width=True, disabled=params_are_different):
                ...
                # 다운로드 시에는 반드시 마지막으로 검색했던 조건을 사용
                temp_csv_path, _, error = save_all_patents_to_csv(st.session_state.api_key, last_searched_params, progress_callback=update_progress)
